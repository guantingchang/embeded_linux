...
#define SENROR_INT_DATA_GET(ret, index, str, member) \
	ret = file_data_read(file_path[index], str); \
	dev->member = atoi(str);\
...
struct icm20608_dev {
	...
	float gyro_x_act, gyro_y_act, accel_z_act;
	float temp_act;
};

struct icm20608_dev icm20608;

static int file_data_read(chr *filename, char *str)
{
	int ret = 0;
	FILE *data_stream = fopen(filename, "r");
