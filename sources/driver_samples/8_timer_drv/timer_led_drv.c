...
{
    struct gpio_led_dev *dev = (struct gpio_led_dev *)(filp->private_data);
    int timer_period;
    unsigned long flags;
...
void timer_cb(struct timer_list *arg)
{
    struct gpio_led_dev *dev = from_timer(dev,arg,timer);
    static int sta = 1;
    int timer_period;
    unsigned long flags;
...
static int __init led_init(void)
{
    ...
    timer_setup(&gpio_led.timer, timer_cb, 0);
    return 0;
    destroy_class:
...
static void __exit led_exit(void)
{
    ...
    del_timer_sync(&gpio_led.timer);
    #if 0
        del_timer(&gpio_led.timer);
    #endif
    printk("led exit\r\n");
    return;
}
